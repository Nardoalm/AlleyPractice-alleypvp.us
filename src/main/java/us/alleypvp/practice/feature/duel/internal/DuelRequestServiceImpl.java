package us.alleypvp.practice.feature.duel.internal;
...
public class DuelRequestServiceImpl implements DuelRequestService {
    ...
    private boolean isRequestInvalid(Player sender, Profile senderProfile, Player finalTarget, boolean isPartyDuel) {
        ...
        if (senderProfile.getState() != ProfileState.LOBBY) {
            sender.sendMessage(AlleyPractice.getInstance().getService(LocaleService.class).getString(GlobalMessagesLocaleImpl.ERROR_YOU_MUST_BE_IN_LOBBY));
            return true;
        }
    ...
    private boolean isValidAcceptRequest(DuelRequest duelRequest) {
        ...
        if (profile.getState() != ProfileState.LOBBY) {
            duelRequest.getSender().sendMessage(AlleyPractice.getInstance().getService(LocaleService.class).getString(GlobalMessagesLocaleImpl.ERROR_YOU_MUST_BE_IN_LOBBY));
            return false;
        }
