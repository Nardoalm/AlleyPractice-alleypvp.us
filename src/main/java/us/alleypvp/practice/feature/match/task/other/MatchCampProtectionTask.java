...
public class MatchCampProtectionTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        Profile profile = AlleyPractice.getInstance().getService(ProfileService.class).getProfile(this.player.getUniqueId());
        ...
        if (
                this.player.getLocation().getY() <= CAMP_Y_LEVEL + 3
                || gamePlayer.isDead()
        ...
        TitleReflectionServiceImpl titleReflectionService = AlleyPractice.getInstance().getService(ReflectionService.class).getReflectionService(TitleReflectionServiceImpl.class);
