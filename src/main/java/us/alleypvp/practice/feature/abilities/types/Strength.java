    ...
    private void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (profile.getCooldown(Strength.class).onCooldown(player)) {
                player.sendMessage(CC.translate("&fVocê está no cooldown de &b&lStrength &7por &4" + DurationFormatter.getRemaining(profile.getCooldown(Strength.class).getRemainingMillis(player), true, true)));
