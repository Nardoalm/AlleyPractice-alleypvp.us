...
public class CC {
    ...
    static {
        ...
        String pluginName;
        try {
            pluginName = AlleyPractice.getInstance().getDescription().getName();
        } catch (Exception e) {
            pluginName = "AlleyPractice"; // Nome padrão para quando o Bukkit estiver offline
        }

        PREFIX = translate("&f[&b" + pluginName + "&f] &r");
