package us.alleypvp.practice.common.bstats.internal;

import us.alleypvp.practice.AlleyPractice;
import us.alleypvp.practice.bootstrap.annotation.Service;
import us.alleypvp.practice.common.bstats.BStatsService;
import org.bstats.bukkit.Metrics;
import us.alleypvp.practice.bootstrap.KaosContext;

/**
 * @author Remi
 * @project kaos-practice
 * @date 27/07/2025
 */
@Service(provides = BStatsService.class, priority = 1)
public class BStatsServiceImpl implements BStatsService {
    @Override
    public void setup(KaosContext context) {
        Metrics metrics = new Metrics(AlleyPractice.getInstance(), 26678);
    }
}
