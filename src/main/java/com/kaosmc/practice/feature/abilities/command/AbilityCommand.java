package com.kaosmc.practice.feature.abilities.command;
...
public class AbilityCommand extends BaseCommand {
    ...
    private KaosPractice plugin = KaosPractice.getInstance();
        ...
        switch (args[0].toLowerCase()) {
            case "give":
                ...
                plugin.getService(AbilityService.class).getAbilityKeys().forEach(ability -> {
                    String displayName = KaosPractice.getInstance().getService(ConfigService.class).getAbilityConfig().getString(ability + ".ICON.DISPLAYNAME");
