    ...
    private void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (profile.getCooldown(Strength.class).onCooldown(player)) {
                player.sendMessage(CC.translate("&fVocê está no cooldown de &6&lStrength &7por &4" + DurationFormatter.getRemaining(profile.getCooldown(Strength.class).getRemainingMillis(player), true, true)));
        ...
        if (action.equals(Action.LEFT_CLICK_AIR) || action.equals(Action.LEFT_CLICK_BLOCK)) {
            ...
            if (isAbility(player.getItemInHand())) {
                if (this.hasCooldown(player)) {
                    player.sendMessage(CC.translate("&fVocê está em cooldown por &4" + DurationFormatter.getRemaining(profile.getCooldown(Strength.class).getRemainingMillis(player), true)));
                    event.setCancelled(true);
                    player.updateInventory();
                }
            }
